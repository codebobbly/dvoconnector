...
use RGU\Dvoconnector\Domain\Repository\AssociationRepository;
    ...
    public function addConfigEvent(&$params, &$pObj)
    {
        ...
        if (!empty($aID)) {
            ...
            foreach ($events->getEvents() as $event) {
                $params['items'][] = [
          implode(' - ', [$event->getStartDate()->format('d.m.Y H:i:s'), $event->getTitle()]),
          $event->getID()
        ];
            }
        }
    ...
    public function addConfigAnnouncement(&$params, &$pObj)
    {
        ...
        if (!empty($aID)) {
            ...
            foreach ($announcements->getAnnouncements() as $announcement) {
                $params['items'][] = [
          implode(' - ', [$announcement->getCreatedDate()->format('d.m.Y H:i:s'), $announcement->getTitle()]),
          $announcement->getID()
        ];
            }
        }
    ...
    public function addConfigFunctionary(&$params, &$pObj)
    {
        ...
        if (!empty($aID)) {
            ...
            foreach ($functionaries->getfunctionaries() as $functionary) {
                $params['items'][] = [
          implode(' | ', [implode(' ', [$functionary->getLastName(), $functionary->getFirstName()]), $functionary->getRole()]),
          $functionary->getID()
        ];
            }
        }
